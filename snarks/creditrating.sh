...
# loadRatings "creditratings.json"

BUYER1_HASH=$(echo -n "$BUYER1$BUYER1_PUB$BUYER1_RATING" | xxd -r -p | sha256sum | cut -d' ' -f1)
BUYER2_HASH=$(echo -n "$BUYER2$BUYER2_PUB$BUYER2_RATING" | xxd -r -p | sha256sum | cut -d' ' -f1)
BUYER3_HASH=$(echo -n "$BUYER3$BUYER3_PUB$BUYER3_RATING" | xxd -r -p | sha256sum | cut -d' ' -f1)
BUYER4_HASH=$(echo -n "$BUYER4$BUYER4_PUB$BUYER4_RATING" | xxd -r -p | sha256sum | cut -d' ' -f1)
